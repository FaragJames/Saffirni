...
export default function OneTrip({
  ...
  const handleCompleteReservation = async () => {
    try {
      const apiResponse = (
        await apiClient.get<GenericApiResponse<TemporaryReservationResponse>>(
  ...
  async function handleTripRating() {
    try {
      const apiResponse = (
        await apiClient.post<ApiResponse>("/API/UserReservationSeat/RateTrip", {
    ...
    try {
      ...
      if (apiResponse.isSuccess) {
        ...
        setUserReservationsState((u) =>
          u?.filter(
            (value) => value.reservationId !== userReservation.reservationId
          )
        );
      } else {
        apiResponse.errors?.forEach((error) => toast.error(error));
            ...
            <DialogContent>
              {dialogContent === DialogType.TripDetails && (
                <div>
                  ...
                  <p>اسم الشركة: {tripInfoRef.current?.companyName}</p>
                  <p>نقطة الانطلاق: {tripInfoRef.current?.source}</p>
                  <p>الوجهة: {tripInfoRef.current?.destination}</p>
                  <p>
                    موعد الانطلاق المتوقع:{" "}
                    {toArabicDateTime(tripInfoRef.current?.expectedDepartTime)}
                  </p>
                  <p>
                    موعد الوصول المتوقع:{" "}
                    {toArabicDateTime(tripInfoRef.current?.expectedArrivalTime)}
                  </p>
                  {!isCurrentReservationsRef.current && (
                    <>
                      <p>
                        ...
                        {toArabicDateTime(
                          tripInfoRef.current?.actualDepartTime
                        )}
                      </p>
                      <p>
                        موعد الوصول الفعلي:{" "}
                        {toArabicDateTime(
                          tripInfoRef.current?.actualArrivalTime
                        )}
                      </p>
                    </>
                  )}
                  <p>نوع الرحلة: {tripInfoRef.current?.busType}</p>
                  <p>سعر التذكرة: {tripInfoRef.current?.ticketPrice}</p>
                </div>
              )}
              {dialogContent === DialogType.ReservationDetails && (
                <div>
                  <Typography sx={{ fontSize: "25px", fontWeight: "700" }}>
                    تفاصيل الحجز:
                  </Typography>
                  <p>
                    صاحب الحجز: {reservationInfoRef.current?.reservationOwner}
                  </p>
                  <p>
                    طريقة الدفع: {reservationInfoRef.current?.paymentMethod}
                  </p>
                  <p>
                    مصدر الحجز: {reservationInfoRef.current?.reservationSource}
                  </p>
                  <p>
                  ...
                  {Object.entries(
                    reservationInfoRef.current?.seatNumberToUserName as Map<
                      number,
                      string
                    >
                  ).map(([key, value]) => (
                ...
                <div>
                  ...
                  >
                    <Rating
                      ...
                      onChange={(_, newValue) => {
                        rating.current = newValue ? newValue : 1;
                      }}
                      size="large"
                    />
                  </div>
